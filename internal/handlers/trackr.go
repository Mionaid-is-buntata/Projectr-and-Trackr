...
func updateProjectHandler(deps *TrackrDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := deps.Service.UpdateMetadata(id, req.GiteaURL, req.LiveURL, req.Notes); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, stage := range stageOrder {
			...
			for _, p := range items {
				...
				body.WriteString(`<a href="/trackr/` + strconv.FormatInt(p.ID, 10) + `">` + esc(p.BriefTitle) + `</a>`)
				if p.BriefComplexity != "" {
					body.WriteString(` <span class="badge">` + esc(p.BriefComplexity) + `</span>`)
				}
		...
		if len(projects) == 0 {
			body.WriteString(`<p class="empty">No projects yet. Generate briefs in Projctr first.</p>`)
		}
