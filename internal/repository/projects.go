...
func (s *ProjectStore) Insert(p *models.Project) (int64, error) {
	res, err := s.db.Exec(`
		INSERT INTO projects (
			brief_id, stage, title, complexity,
			repository_url, linkedin_url,
			gitea_url, live_url, linkedin_draft, notes,
			date_created, date_selected, date_started,
			date_completed, date_published, date_parked
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		p.BriefID, p.Stage, p.Title, p.Complexity,
		p.RepositoryURL, p.LinkedInURL,
...
func (s *ProjectStore) GetByID(id int64) (*models.Project, error) {
	...
	err := s.db.QueryRow(`
		SELECT id, brief_id, stage, title, complexity,
			repository_url, linkedin_url,
			gitea_url, live_url, linkedin_draft, notes,
			date_created, date_selected, date_started,
			date_completed, date_published, date_parked
		...
		&p.ID, &p.BriefID, &p.Stage, &p.Title, &p.Complexity,
		&p.RepositoryURL, &p.LinkedInURL,
...
func (s *ProjectStore) List() ([]*models.ProjectWithBrief, error) {
	rows, err := s.db.Query(`
		SELECT p.id, p.brief_id, p.stage, p.title, p.complexity,
			p.repository_url, p.linkedin_url,
		...
		FROM projects p
		LEFT JOIN briefs b ON p.brief_id = b.id
	...
	for rows.Next() {
		...
		var dateSelected, dateStarted, dateCompleted, datePublished, dateParked sql.NullTime
		var giteaURL, liveURL, linkedInDraft sql.NullString
		var briefTitle, briefComplexity sql.NullString
		if err := rows.Scan(
			&pw.ID, &pw.BriefID, &pw.Stage, &pw.Title, &pw.Complexity,
			&pw.RepositoryURL, &pw.LinkedInURL,
			&giteaURL, &liveURL, &linkedInDraft, &pw.Notes,
			&pw.DateCreated, &dateSelected, &dateStarted,
			&dateCompleted, &datePublished, &dateParked,
			&briefTitle, &briefComplexity,
		); err != nil {
			return nil, err
		}
		...
		if briefTitle.Valid {
			pw.BriefTitle = briefTitle.String
		}
...
func (s *ProjectStore) Clear() error {
...
func (s *ProjectStore) Update(p *models.Project) error {
	_, err := s.db.Exec(`
		UPDATE projects SET
			...
			gitea_url = ?, live_url = ?, linkedin_draft = ?, notes = ?
		WHERE id = ?`,
		p.Title, p.Complexity, p.GiteaURL, p.LiveURL, p.LinkedInDraft, p.Notes, p.ID,
	)
	return err
}
